...
@property (nonatomic, copy) RCTDirectEventBlock onPopUpLoad;
#if !TARGET_OS_OSX
@property (nonatomic, copy) WKWebView *webView;
#else
...
@property (strong, nonatomic) IBOutlet WKWebView *pwebView;
@property (strong, nonatomic) IBOutlet UIButton *closeBtn;
@property (strong, nonatomic) IBOutlet UIButton *popUpBackBtn;
@property (strong, nonatomic) IBOutlet UIView *popupToolBarView;
@end
...
//}
    ...
    CGRect popUpToolBarFrame = CGRectMake(5, self.webView.frame.origin.y+5, self.webView.frame.size.width-10, 40); //popUpToolBarFrame
    self.popupToolBarView = [[UIView alloc]initWithFrame:popUpToolBarFrame];
    
    CGRect centerCloseBtnFrame = CGRectMake(self.popupToolBarView.frame.size.width/2 -60, 0, 120, 40); //Center Close Btn
//    CGRect fullCloseBtnFrame = CGRectMake(5, 0, self.popupToolBarView.frame.size.width-10, 40); //FullClose Btn
//    CGRect rightCloseBtnFrame = CGRectMake(self.popupToolBarView.frame.size.width-70, 0, 60, 40);//left close Btn
    CGRect leftCloseBtnFrame = CGRectMake(0, 0, 60, 40);//left close Btn
    ...
    [self.closeBtn setFrame:centerCloseBtnFrame];
    [self.closeBtn setTitle:@"Close" forState:UIControlStateNormal];
//    [self.closeBtn setTitle:@"x" forState:UIControlStateNormal]; ////left close Btn
    [self.closeBtn.titleLabel setFont:[UIFont fontWithName:@"Helvetica-Bold" size:16.0]];
    [self.closeBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [self.closeBtn addTarget:self action:@selector(PopUpWebviewCloseButtonClicked:) forControlEvents:UIControlEventTouchUpInside];
    NSString *themeColor = [ReactNativeConfig.env valueForKey:@"THEME_COLOR"];
    if (themeColor) {
        self.closeBtn.backgroundColor = [RNCWebView colorWithHexString:themeColor];
        self.popupToolBarView.backgroundColor = [RNCWebView colorWithHexString:themeColor];
    }else{
        self.closeBtn.backgroundColor = [UIColor grayColor];
        self.popupToolBarView.backgroundColor = [UIColor grayColor];
    }
    
    //Add BackButton
    self.popUpBackBtn = [UIButton buttonWithType: UIButtonTypeCustom];
    [self.popUpBackBtn setFrame:leftCloseBtnFrame];
    self.popUpBackBtn.hidden = YES;
    [self.popUpBackBtn setTitle:@"<" forState:UIControlStateNormal];
//    [self.closeBtn setTitle:@"x" forState:UIControlStateNormal]; ////left close Btn
    [self.popUpBackBtn.titleLabel setFont:[UIFont fontWithName:@"Helvetica-Bold" size:16.0]];
    [self.popUpBackBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [self.popUpBackBtn addTarget:self action:@selector(PopUpWebviewBackButtonClicked:) forControlEvents:UIControlEventTouchUpInside];
    
    if (themeColor) {
        self.popUpBackBtn.backgroundColor = [RNCWebView colorWithHexString:themeColor];
    }else{
        self.popUpBackBtn.backgroundColor = [UIColor grayColor];
    }
    CGRect ppwebviewrect = CGRectMake(webView.frame.origin.x+5, webView.frame.origin.y+42, webView.frame.size.width-10, webView.frame.size.height-44);
    if(_showCustomBottomBar){
        ppwebviewrect = CGRectMake(webView.frame.origin.x+5, webView.frame.origin.y+42, webView.frame.size.width-10, webView.frame.size.height);
    }
    self.pwebView =[[WKWebView alloc] initWithFrame:ppwebviewrect configuration:configuration];
    self.pwebView.navigationDelegate = self;
    self.pwebView.UIDelegate = self;
    self.pwebView.layer.borderColor = [[RNCWebView colorWithHexString:themeColor] CGColor];
    self.pwebView.layer.borderWidth = 2.0;

//    [self addSubview:self.closeBtn];
    [self addSubview:self.pwebView];
    [self bringSubviewToFront:self.pwebView];
//    [self bringSubviewToFront:self.closeBtn];
    
    
    [self addSubview:self.popupToolBarView];
    [self.popupToolBarView addSubview:self.closeBtn];
    [self.popupToolBarView bringSubviewToFront:self.closeBtn];
    
    
    //AddBack Button
    [self.popupToolBarView addSubview:self.popUpBackBtn];
    [self.popupToolBarView bringSubviewToFront:self.popUpBackBtn];
...
- (void)webViewDidClose:(WKWebView *)webView {
    _onPopUpLoad([self PopupBaseEvent:false]);
    self.pwebView.hidden = YES;
    self.closeBtn.hidden = YES;
    [self.pwebView removeFromSuperview];
    [self.closeBtn removeFromSuperview];
    self.webView.hidden = false;
    
    self.popupToolBarView.hidden = YES;
    [self.popupToolBarView removeFromSuperview];
    
    self.popUpBackBtn.hidden = YES;
    [self.popUpBackBtn removeFromSuperview];
    
    [self bringSubviewToFront:self.webView];
}
-(void) PopUpWebviewBackButtonClicked:(UIButton*)sender{
...
-(void) PopUpWebviewCloseButtonClicked:(UIButton*)sender{
    _onPopUpLoad([self PopupBaseEvent:false]);
    self.webView.hidden = false;
    self.pwebView.hidden = YES;
    self.closeBtn.hidden = YES;
...
- (NSMutableDictionary<NSString *, id> *)PopupBaseEvent:(BOOL)isPopUpClosed
{
    NSMutableDictionary *event = [[NSMutableDictionary alloc]init];
    if (isPopUpClosed) {
        [event setValue:@"true" forKey:@"isPopUpClosed"];
    }else{
        [event setValue:@"false" forKey:@"isPopUpClosed"];
    }
  ...
  didFinishNavigation:(WKNavigation *)navigation
{
    if (webView == self.pwebView) {
        ...
        _onPopUpLoad([self PopupBaseEvent:true]);
